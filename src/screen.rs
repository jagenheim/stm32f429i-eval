...
pub struct Stm32F4EvalDisplay<T: 'static + SupportedWord> {
...
impl<T: 'static + SupportedWord> Stm32F4EvalDisplay<T> {
    pub fn new(ltdc: LTDC, dma2d: DMA2D, pins: LtdcPins) -> Stm32F4EvalDisplay<T> {
        ...
        Stm32F4EvalDisplay { controller }
    }
}

impl DrawTarget for Stm32F4EvalDisplay<u16> {
    type Color = Rgb565;
...
impl OriginDimensions for Stm32F4EvalDisplay<u16> {
    fn size(&self) -> Size {
        Size::new(480, 272)
    }
}
